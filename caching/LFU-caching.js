...
LfuCache.prototype.set = function(key, value) {
    ...
    if (node === undefined) {
        ...
        // if the node exists, we need to get it and move it to the new linked list
            ...
            if (oldFreqCount == this.minFrequency && Object.keys(this.frequency[oldFreqCount]).size === 0) {
                this.minFrequency++;
            }
        }
}
...
LfuCache.prototype.get = function(key) {
    ...
    if (oldFreqCount === this.minFrequency && Object.keys(this.frequency[oldFreqCount]).length === 0) {
        this.minFrequency++;
    }
    return node.data;
}
