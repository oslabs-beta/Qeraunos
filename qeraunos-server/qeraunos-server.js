...
//Setting = O(1)
//Deleteing = O(1)
var QeraunosServer = function (capacity) {
    this.keys = {};
    this.frequency = {};
...
QeraunosServer.prototype.removeAtTail = function (DLL) {
...
QeraunosServer.prototype.addNode = function (key, value) {
    var node = new serverNode(key, value);
    // place node into hashtable
    this.keys[key] = node;
    // check if frequency of 1 exists already, if not, create a new list with freq of 1
    if (!this.frequency[1])
        this.frequency[1] = new serverDLL(1);
...
QeraunosServer.prototype.insertAtHead = function (newNode, freqList) {
...
QeraunosServer.prototype.updateNode = function (node) {
    ...
    if (!this.frequency[node.freqCount])
        this.frequency[node.freqCount] = new serverDLL(node.freqCount);
...
QeraunosServer.prototype.removeNode = function (newNode, freqList) {
...
QeraunosServer.prototype.get = function (key) {
    if (!this.keys[key])
        return undefined;
...
QeraunosServer.prototype.set = function (key, value) {
